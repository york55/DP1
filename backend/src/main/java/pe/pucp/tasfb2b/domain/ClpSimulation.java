...
@Entity
@Table(name = "Clp_simulations")
@Getter
@Setter
@NoArgsConstructor
public class ClpSimulation {
    ...
    private ScenarioType scenarioType = ScenarioType.COLLAPSE;
    ...
    @Column(name = "days_simulated", nullable = false)
    private int daysSimulated = 0;

    @Column(name = "last_read_until")
    private LocalDateTime lastReadUntil;

    @Column(name = "collapsed_at")
    private LocalDateTime collapsedAt;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "collapsed_airport_id")
    private ClpAirport collapsedAirport;
