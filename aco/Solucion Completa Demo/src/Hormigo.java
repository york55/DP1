...
public class Hormigo {
    ...
    public void construirSolucion(){
        ...
        while(!origen.equals(destino) && factible && pasos < maxPasos){
            ...
            List<Double> probabilidades = obtenerProbabilidades(posibles);
    ...
    /*
    private List<VueloDiario> obtenerVuelosPosibles(Envio envio,boolean mismoContinente,String origen,String destino,
        ...
        for(VueloDiario vuelo: vuelos_origen){
            ...
            //plazo maximo de 1 dia (mismo continente)
            //plazo maximo de 2 dias (distinto continente)
            LocalDateTime salida = LocalDateTime.of(
                    tiempoActual.toLocalDate(),
                    vuelo.getHoraSalida()
            );
            if(salida.isBefore(tiempoActual)){
   ...
   private List<VueloDiario> obtenerVuelosPosibles(Envio envio, boolean mismoContinente, String origen, String destino,
        ...
        for (VueloDiario vuelo : vuelos_origen) {
            ...
            if (visitados.contains(vuelo_destino)) continue;
            // Calcular momento de salida real (puede ser hoy o mañana)
            LocalDateTime salida = LocalDateTime.of(tiempoActual.toLocalDate(), vuelo.getHoraSalida());
            if (salida.isBefore(tiempoActual)) {
                salida = salida.plusDays(1);
            }
            // Calcular momento de llegada (puede cruzar medianoche)
            LocalDateTime llegada = LocalDateTime.of(salida.toLocalDate(), vuelo.getHoraLlegada());
            if (vuelo.getHoraLlegada().isBefore(vuelo.getHoraSalida())) {
                llegada = llegada.plusDays(1);
            }
            ...
            VueloFecha vf = vuelo.getVueloFecha(salida.toLocalDate());
    ...
    private List<Double> obtenerProbabilidades(List<VueloDiario> vuelos){
