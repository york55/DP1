...
/**
 * FlightCancelPanel — floating button + expandable panel for cancelling flights.
 *
 ...
 * - Badge counter uses a separate lightweight ref counter
 */
export default function FlightCancelPanel() {
  ...
  useEffect(() => {
    // Count cancelled on mount and then every 6s
    const count = () => {
  ...
  const pageData = useMemo(() =>
    activeList.slice(safePage * PAGE_SIZE, (safePage + 1) * PAGE_SIZE),
    [activeList, safePage]
  )
  ...
  const handleConfirmCancel = useCallback(async () => {
    ...
    try {
      ...
      // Update snapshot + badge immediately
                ...
                <tr style={{ position: 'sticky', top: 0, backgroundColor: '#fff', zIndex: 1 }}>
                  {tab === 0 ? (
                    <>
                      ...
                      <th style={thStyle}>Salida UTC</th>
